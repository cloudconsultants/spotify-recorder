...
from os import getenv, makedirs, system, fsync
import subprocess
...
from recorder import SpotifyRecorder
...
class sp_instance:
    ...
    def record_multiple(self, type, info, args):
        ...
        for track_info in trackinfolist:
            ...
            self.record_manager(track_info, fpath, args, fname=fname, record=(not args.no_record), playlist_name=playlist_name, parent_album=info)

        # Cleanup: Kill Spotify after all tracks in album/playlist are complete
        print("\n[+] Album/playlist complete. Stopping Spotify...")
        subprocess.run("pkill -f spotify 2>/dev/null || true", shell=True)

    def record_manager(self, track_info, folder_path, args=None, fname=None, record=True, playlist_name=None, parent_album=None):
        ...
        if record:
            if not file_exists or args.overwrite:
                ...
                # Get absolute path to spotdl.sh
                script_dir = Path(__file__).parent
                spotdl_path = script_dir / 'spotdl.sh'
                spotdl_cmd = f"{spotdl_path} {uri} \"{filepath}\" {duration_s} {'1' if self.verbose else ''}"

                try:
                    subprocess.run(spotdl_cmd, shell=True, check=True)

                    # Add file synchronization to prevent race condition
                    try:
                        # Small delay to allow file system to finish writing
                        time.sleep(0.2)
                        ...
                        for attempt in range(max_attempts):
                            try:
                                ...
                                if attempt == max_attempts - 1:
                                    DINFO(f"Warning: File {filepath} may still be being written: {e}")
                                else:
                                    time.sleep(0.1)
                                    continue

                    except Exception as e:
                        DINFO(f"Warning: File synchronization issue for {filepath}: {e}")
    ...
    def set_track_filename(self, track_info, folder_path=None):
        ...
        # Format: "01 - Song Name.mp3"
        track_clean = self.fix_filename(track_name)
        filename = f"{track_number:02d} - {track_clean}.mp3"

        if folder_path:
            # For playlists: use provided folder path (already has "songs/" prefix)
            filepath = f"{folder_path}/{filename}"
        else:
            # For single tracks/albums: songs/Artist/Album/Track.mp3
            artist = self.fix_filename(track_info['artists'][0]['name'])
            album = self.fix_filename(track_info['album']['name'])
            filepath = f"songs/{artist}/{album}/{filename}"

        return filepath

    def set_fpath(self, type, info):
        if type == "playlist":
            author = info['owner']['display_name']
            name = info['name']
            fname = self.fix_filename(f"{author}_-_{name}")
            fpath = f"{OUT_FOLDER}/{fname}"
        else:  # album
            # Use Artist/Album structure for albums
            artist = self.fix_filename(info['artists'][0]['name'])
            album = self.fix_filename(info['name'])
            fname = f"{artist}/{album}"
            fpath = f"{OUT_FOLDER}/{fname}"
        return fname, fpath
...
* uri : {info['uri']}""")
    ...
    # Edit music metadata
    def edit_metadata(self, filepath, track_info, playlist_name=None, parent_album=None):
        if self.verbose:
            DINFO("Edit metadata")
        ...
        # Update album artist - use parent album data as fallback
        album_data = track_info.get('album', parent_album if parent_album else {})
        album_artists = album_data.get('artists', track_info.get('artists', []))
        if album_artists:
            f.setall('TPE2', [TPE2(text=', '.join([artist['name'] for artist in album_artists]))])

        # Update album - use parent album data as fallback
        album_name = album_data.get('name', 'Unknown Album')
        album_type = album_data.get('album_type', 'album')
        album = album_name + (" - single" if album_type == "single" else "")
        ...
        # Update TSRC - use defensive access
        external_ids = track_info.get('external_ids', {})
        isrc = external_ids.get('isrc', '')
        if isrc:
            f.setall('TSRC', [TSRC(encoding=3, text=isrc)])

        # Update Genre - use defensive access
        artists_data = track_info.get('artists', [])
        if artists_data and "genres" in artists_data:
            genres = ', '.join(g for g in artists_data["genres"])
            if genres:
...
#           python3 {argv[0]} --search \"song name author\"  [options]""")
    ...
    if args.search:
        ...
        sp.record_manager(track_info, ".", args, record=(not args.no_record), parent_album=None)
